...
class ImageAligner(BaseImageOperator):
    ...
    def operate(self, src_img: np.ndarray, dst_img: np.ndarray) -> dict:
        ...
        src_img_t, dst_img_t = self.image_transformer.fit_transform([src_img, dst_img])
    ...
    def find_homography(
        ...
        src_pts = np.array([src_kp[m.queryIdx].pt for m in matches]).reshape(-1, 1, 2)
        dst_pts = np.array([dst_kp[m.trainIdx].pt for m in matches]).reshape(-1, 1, 2)

        LOGGER.info("Finding transform...")

        # Get the transform
        M, mask = cv2.findHomography(src_pts, dst_pts, **self.find_homography_options)
    ...
    def validate_warp(
        self, dst_img: np.ndarray, x_off: int, y_off: int, x_size: int, y_size: int
    ):
        ...
        if (
            ...
            raise exceptions.OutOfBoundsError("Warping results in out-of-bounds image")
    ...
    def validate_brightness(self, img: np.ndarray, error_type: str = "src"):
        ...
        if bright_area < self.required_bright_pixel_area:
            if error_type == "src":
                ...
                raise KeyError("Unrecognized error type in validate_brightness")
    ...
    def validate_homography(self, M: np.ndarray):
        ...
        if M is None:
            raise exceptions.HomographyTransformError("Transform matrix M is None")

        abs_det_M = np.abs(np.linalg.det(M))

        det_in_range = (abs_det_M > self.det_min) and (abs_det_M < self.det_max)
...
class ImageOperatorQueue:
    ...
    def operate(self, src_img: np.ndarray, dst_img: np.ndarray) -> dict:
        ...
        for i, image_joiner in enumerate(self.image_joiners):
            ...
            result_code, result, log = image_joiner.process(src_img, dst_img)
